...
# Creating the dataset by first creating the respective directories
...
while True:
    ...
    # displaying the image that is going to be processed later
    cv2.imshow("Frame", frame)

    roi = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    _, roi = cv2.threshold(roi, 120, 255, cv2.THRESH_BINARY)
    cv2.imshow("ROI", roi)
    # Capturing the images based on the key pressed.
